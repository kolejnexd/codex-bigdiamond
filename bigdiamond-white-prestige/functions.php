...
function bdwp_save_engraving_to_cart_item( array $cart_item_data, int $product_id, int $variation_id ): array {
	if ( isset( $_POST['bdwp_engraving_text'] ) && ! empty( $_POST['bdwp_engraving_text'] ) ) {
		// 🛡️ Sentinel: Always use wp_unslash() on superglobals before sanitizing to prevent double-escaping from wp_magic_quotes().
		$engraving_text = sanitize_text_field( wp_unslash( (string) $_POST['bdwp_engraving_text'] ) );
